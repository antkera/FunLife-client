...
export default function SendForm({setDisplayType}) {
  ...
  const handleNewMessage = async(e) => {
    ...
    try {
        const response = await service.post("/messages/message",{receiver: receiver.value, message: message.value} )
    ...
    <form className="flex column borderRadius" onSubmit={handleNewMessage}>
      <div>
        ...
        <select className="borderRadius" name="receiver">
          {friendsArr.map((each) => {
            return <option key={each._id} value={each._id}>{each.username}</option>;
          })}
        </select>
      </div>
      
      <textarea required= {true} className="borderRadius margin" type="text" name="message"/>
      <p></p>
